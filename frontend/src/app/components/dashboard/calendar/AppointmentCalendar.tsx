...
export function AppointmentCalendar({
  ...
  const filteredAppointments = useMemo(() => {
    return appointments.filter((apt) => {
      ...
      if (selectedDepartment && apt.department !== selectedDepartment) return false;
      return true;
    });
  ...
  if (view === "day") {
    return (
      ...
      >
        ...
        {/* Timeline */}
        <div ref={scrollRef} className="max-h-[600px] overflow-y-auto dash-scrollbar">
          {HOURS.map((hour) => {
            ...
            return (
              <div key={hour} className="flex border-b border-dash-border last:border-b-0">
                {/* Time label */}
          ...
          {dayAppointments.length === 0 && (
            <div className="flex flex-col items-center justify-center py-16 text-slate-400 dark:text-slate-500">
              ...
              <p className="mt-1 text-xs">Schedule a new appointment to get started.</p>
            </div>
          )}
        </div>
      </motion.div>
  ...
  if (view === "week") {
    return (
      ...
      >
        ...
        {/* Week timeline */}
        <div ref={scrollRef} className="max-h-[560px] overflow-y-auto dash-scrollbar">
          {HOURS.map((hour) => (
            <div key={hour} className="grid grid-cols-8 border-b border-dash-border last:border-b-0">
              {/* Hour label */}
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      isToday(date) && "bg-dash-primary-light/30 dark:bg-dash-primary/10",
                    )}
                  >
                    {hourApps.map((apt) => (
                      <AppointmentEvent
    ...
    >
      ...
      <div className="sticky top-0 z-10 grid grid-cols-7 border-b border-dash-border bg-white dark:bg-slate-800">
        {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => (
          <div key={day} className="py-2 text-center text-[11px] font-semibold text-slate-500 dark:text-slate-400">
            {day}
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-7">
        {monthAppointments.map(({ date, count, appointments: dayApps }, idx) => {
          if (!date) {
            return <div key={`empty-${idx}`} className="min-h-[100px] border-b border-r border-dash-border bg-slate-50/50 dark:bg-slate-800/30" />;
          }
          ...
          return (
            <div
              ...
              className={cn(
                ...
                isCurrentMonth && !isSelected && !today && "hover:bg-slate-50 dark:hover:bg-slate-700/50",
                today && !isSelected && "bg-dash-primary-light/20 dark:bg-dash-primary/10",
                isSelected && "bg-dash-primary-light/40 ring-1 ring-inset ring-dash-primary dark:bg-dash-primary/20",
              )}
              role="button"
              tabIndex={0}
              ...
              <div className="mb-1 flex items-center justify-between">
                <span
                  className={cn(
                    ...
                    today ? "bg-dash-primary text-white" : isCurrentMonth ? "text-slate-800 dark:text-slate-200" : "text-slate-400 dark:text-slate-600",
                  )}
                >
                  {date.getDate()}
                </span>

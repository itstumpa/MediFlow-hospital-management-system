  ...
  return (
    ...
    >
      <div className="absolute right-3 top-3">
        <div
          className={`flex h-10 w-10 items-center justify-center rounded-xl ${color} bg-opacity-10`}
        >
      ...
      <div className="mt-3 h-1 w-full rounded-full bg-slate-100 dark:bg-slate-800">
        <motion.div
          ...
          animate={
            inView
              ? {
                  width: `${Math.min((displayed / (value * 1.5)) * 100, 100)}%`,
                }
              : {}
          }
...
export function DoctorsStats({ doctors }: DoctorsStatsProps) {
  ...
  const availableNow = doctors.filter(
    (d) => d.availability === "Available",
  ).length;
  const avgRating =
    doctors.reduce((acc, d) => acc + d.rating, 0) / totalDoctors;
  const onLeave = doctors.filter(
    (d) => d.status === "On Leave" || d.status === "Vacation",
  ).length;

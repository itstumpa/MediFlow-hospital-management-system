              ...
              {assignedDoctors.map((doctor) => (
                <motion.div
                  ...
                  className={cn(
                    ...
                    doctor.isHead
                      ? "border-dash-primary-light bg-dash-primary-light dark:border-teal-500/30 dark:bg-teal-500/10"
                      : doctor.isAssistantHead
                        ? "border-purple-200 bg-purple-50 dark:border-purple-500/30 dark:bg-purple-500/10"
                        : "border-slate-200 bg-white dark:border-slate-600 dark:bg-slate-800",
                  )}
                >
                  <div className="flex h-9 w-9 items-center justify-center rounded-full bg-dash-primary-light text-xs font-bold text-dash-primary dark:bg-teal-500/20 dark:text-accent">
                    {doctor.name
                      .split(" ")
                      .map((n) => n[0])
                      .join("")
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium text-slate-800 dark:text-slate-200">
                      {doctor.name}
                      {doctor.isHead && (
                        <span className="ml-1.5 inline-flex items-center gap-0.5 rounded-full bg-dash-primary-light px-2 py-0.5 text-[10px] font-semibold text-dash-primary dark:bg-teal-500/20 dark:text-accent">
                          Head
                        </span>
                      )}
      ...
      <div className="space-y-3">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-xl border border-slate-200 bg-white py-2.5 pl-10 pr-4 text-sm transition-all focus:border-dash-primary focus:outline-none focus:ring-2 focus:ring-dash-primary/20 dark:border-slate-600 dark:bg-slate-800"
          />
        </div>
            ...
            filteredDoctors.map((doctor) => {
              ...
              return (
                ...
                >
                  ...
                  {assigned && (
                    <Check className="h-4 w-4 shrink-0 text-dash-primary" />
                  )}
                </motion.button>
              );
            })

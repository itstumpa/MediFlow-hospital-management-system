...
import type { MedicalFilters, RecordType, ViewMode } from "./types";
import { allDepartments, mockRecordDoctors, recordTypeLabels } from "./types";

/* ─── Props ─── */

interface FiltersProps {
  filters: MedicalFilters;
  onChange: (filters: MedicalFilters) => void;
  onReset: () => void;
  viewMode: ViewMode;
...
export function Filters({
  ...
  const update = (patch: Partial<MedicalFilters>) =>
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <select
            ...
            onChange={(e) =>
              update({ recordType: e.target.value as RecordType | "" })
            }
            className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-slate-600 transition-all hover:border-slate-300 focus:border-[var(--color-primary)] focus:outline-none dark:border-slate-600 dark:bg-slate-800 dark:text-slate-300 dark:focus:border-[var(--color-accent)]"
            aria-label="Filter by record type"
          >
            <option value="">All Types</option>
            {(Object.entries(recordTypeLabels) as [RecordType, string][]).map(
              ([key, label]) => (
          ...
          <select
            value={filters.sort}
            onChange={(e) =>
              update({ sort: e.target.value as MedicalFilters["sort"] })
            }
            className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-slate-600 transition-all hover:border-slate-300 focus:border-[var(--color-primary)] focus:outline-none dark:border-slate-600 dark:bg-slate-800 dark:text-slate-300 dark:focus:border-[var(--color-accent)]"
